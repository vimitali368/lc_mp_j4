            ...
            @if(auth()->user()->can('add articles'))
                <div class="row">
                    <div class="col-2 mb-3">
                        <a href="{{ route('personal.article.create') }}" type="button"
                                    ...
                                    <tr>
                                        ...
                                        @if(auth()->user()->can('show articles'))
                                            <td class="text-center">
                                                <a href="{{ route('personal.article.show', $article->id) }}"><i
                                                        class="far fa-eye"></i></a></td>
                                        @endif
                                        @if(auth()->user()->can('edit articles'))
                                            <td class="text-center">
                                                <a href="{{ route('personal.article.edit', $article->id) }}"
                                                   class="text-success"><i class="fas fa-pencil-alt"></i></a></td>
                                        @endif
                                        @if(auth()->user()->can('delete articles'))
                                            <td>
                                                <form action="{{ route('personal.article.delete', $article->id) }}"
                                                      method="POST">
                                                    @csrf
                                                    @method('DELETE')
